...
function LogoLines({ isCard = false }: { isCard?: boolean }) {
  const cls = (extra: string) => `logos_banner-line${isCard ? " is-about" : ""} ${extra}`.trim();
  const lines: string[] = [];
  ...
  return (
    <div className="btn__icon btn__icon--lg w-embed">
      <svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" viewBox="0 0 21 17" fill="none" preserveAspectRatio="xMidYMid meet" role="img">
        ...
        <path d="M20.9048 0.318359V16.682H17.6321V15.7104H19.8182V1.28995H17.6321V0.318359H20.9048Z" fill="currentColor" />
      </svg>
    </div>
  );
}
        ...
        <div className="container-large">
          <div className="about-fun_component">
            <div className="about-fun_layout">
              ...
              <Swiper className="about-card" modules={[EffectCards, Pagination]} effect="cards" grabCursor pagination={{ clickable: true }}>
                {aboutFacts.slides.map((slide, i) => (
                  <SwiperSlide key={i}>
                    <div className="about-fun_card">
                      <div className="about-fun_card-header">
                        <div className="text-caption-1 text-color-teritary">{slide.caption}</div>
                        <h3 className="heading-style-h1 is-card">{slide.title}</h3>
                      </div>
                      <div className="about-fun_element-component">
                        <div className="about-fun_line" />
                        <div className="about-fun_element-wrapper is-card">
                          <LogoLines isCard />
                          <div className="text-caption-2 text-color-teritary">{slide.elementCaption}</div>
                        </div>
                      </div>
                          ...
                          {slide.video ? (
                            ...
                            <>
                              ...
                              {slide.imageClass === "is-plants" ? (
                                ...
                                <img src={slide.image} alt={slide.caption} loading="lazy" className="about-fun_image" />
                              )}
                            </>
                          )}
                        </div>
                        <div className="about-fun_video-misc">
                          <div className="text-caption-2 text-color-teritary">{slide.videoCaption ?? slide.misc}</div>
                        </div>
                        {slide.ctaLabel ? (
